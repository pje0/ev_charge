...
<jsp:include page="admin_dashboard.jsp" />
    ...
    <script>
        // 페이지 로드 시 URL 파라미터를 확인하여 이전 탭 상태를 복원합니다.
        $(document).ready(function() {
            const urlParams = new URLSearchParams(window.location.search);
            const activeTab = urlParams.get('tab'); // URL에서 ?tab=값 읽기
            
            if (activeTab && ['reservation', 'notice', 'inquiry'].includes(activeTab)) {
                fn_switch_tab(activeTab);
            }
        });
